...
def calculate_top(line, dia):
    ...
    if dia_center>=line.main_top:
        dia_distance=dia.bottom-dia_center
        distance_offset=0
        start=dia.bottom-line.main_top+1
        return ((distance_offset), (start), "overlap")
...
def attach_diacritics_to_lines(lines, diacritics, margin, debug_dir, page_img):
    """
    ...
    for dia in diacritics:
        ...
        for key, (lines_array, status) in over_lines.items():
            ...
            for i, current_line in enumerate(lines_array):
                ...
                log(f"beforr col-start {dia.left} and col-end { dia.right}")
                ...
                log(f"endcol-start {col_start_global} and col-end { col_end_global}")
                ...
                local_start = col_start_global 
                local_end   = col_end_global 
                ...
                if status == "above" and i == 0:
                   ...
                   # distance_offset,start, within_Status = calculate_top(current_line, dia)
                    ...
                    kk_range = range(dia.bottom+1, current_line.main_bottom)  # from top (0) downward
                    scan_mode = "above_topline"
                   # new_height=current_line.main_bottom-dia.bottom+1
                    log(f"^^^^^^^^^^^^^^^^^^^ {kk_range} ")
                elif status == "above" and i == 1:
                   # distance_offset ,start, within_Status= calculate_bottom(current_line, dia)
                    kk_range = range( dia.top-1,current_line.main_top ,-1)  # bottom->up
                   ...
                   # new_height=start+1
                    ...
                    #distance_offset ,start, within_Status= calculate_top(current_line, dia)
                    kk_range = range(dia.bottom+1, current_line.main_bottom) # top->down
                   ...
                   # new_height=line_height-start+1
                    ...
                    if i == 0:
                       # distance_offset , start,within_Status= calculate_bottom(current_line, dia)
                        kk_range = range(dia.top-1,current_line.main_top ,-1)
                        scan_mode = "between_top"
                        log(f"^^^^^^^^^^^^^^^^^^^ {kk_range}")
                      #  new_height=start+1
                    elif i == 1:
                       # distance_offset, start,within_Status = calculate_top(current_line, dia)
                        kk_range = range(dia.bottom+1, current_line.main_bottom)
                ...
                for local_x in range(local_start, local_end + 1):
                    ...
                    for kk in kk_range:
                        ...
                        # safety bounds check
                        if kk < 0 or kk >= page_img.shape[0] or local_x < 0 or local_x >= page_img.shape[1]:
                            
                            continue

                        if page_img[kk, local_x] == 0:  # black pixel found
                            ...
                            if status == "between":
                                if i == 0:
                                    ...
                                    dist = dia_distance + (dia.top-kk+1)
                                   # visualize: top line found pixel at kk in local coords
                                    log(f"{dia_distance}  {dist}")
                                    visualize_search(
                                        line=current_line,
                                        dia=dia,
                                        line_img=page_img,
                                    ...
                                    dist = dia_distance+(kk-dia.bottom+1)
                                    log(f"{dia_distance}   {kk}  {dist}")
                                    visualize_search(
                                        line=current_line,
                                        dia=dia,
                                        line_img=page_img,
                                        column=local_x,
                                        dia_top=dia.bottom+1,
                                        dia_bottom=kk
                                    )
                            elif status == "above":
...
def calculate_distance_for_column(dia, column, status="above"):
    ...
    if status == "above":
        ...
        if last_black_y is not None:
            dia_distance = last_black_y - dia.top + 1  # distance from top
        ...
        if last_black_y is not None:
            dia_distance = dia.bottom - last_black_y + 1  # distance from bottom
